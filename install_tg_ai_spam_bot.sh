...
# 自有资源发布机器人：其自动发布内容不参与广告审核（逗号分隔）
TRUSTED_CONTENT_BOT_USERNAMES=wjccbot,ananadsbot,se_resource_bot
...
TRUSTED_CONTENT_BOT_USERNAMES = {x.strip().lower().lstrip("@") for x in os.getenv("TRUSTED_CONTENT_BOT_USERNAMES", "wjccbot,ananadsbot,se_resource_bot").split(",") if x.strip()}
...
async def is_trusted_content_message(msg) -> bool:
    """Trust posts generated by the owner's configured retrieval bots.

    Telegram emits those automated resource posts as user 777000 (Telegram) rather
    than the channel identity, so sender_chat alone cannot protect them.
    """
    if getattr(msg, "from_user", None) and msg.from_user.id != 777000:
        return False
    content = message_content(msg).lower()
    if not content:
        return False
    linked = set(re.findall(r"t\.me/([a-z0-9_]{5,})(?:\?|/)", content, re.I))
    return bool(linked & TRUSTED_CONTENT_BOT_USERNAMES)
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_sender_chat_message(msg) and getattr(msg.sender_chat, "id", None) == chat.id:
        return
    if await is_trusted_content_message(msg):
        log.info("trusted content-bot post skipped chat=%s message=%s", chat.id, msg.message_id)
        return
